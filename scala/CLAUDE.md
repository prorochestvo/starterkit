...
3. scala-reviewer × 3 (run in parallel — single message, three tool calls)
   ...
   accepts as a trade-off), and presents the merged punch list to the user.
    ↓
  ❌ Blocker/Major found?  → Back to scala-engineer with the consolidated findings.
...
| `scala-reviewer` (×3, parallel) | Lens-specific verdicts, severity-ranked findings, patch sketches | Three independent review reports |
