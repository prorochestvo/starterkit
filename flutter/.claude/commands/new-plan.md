...
2. Determine the next plan number `NNN`:
   ...
   - Extract the highest numeric prefix across all three directories (formats `NNN-*.md` and `YYMMDD.NNNN.*.md`). For `YYMMDD.NNNN`, only consider the `NNNN` portion if it represents a plan-number rather than a daily index — i.e. ignore `YYMMDD.NNNN.*` entirely when computing the next active number (those use a daily index, not a global one).
   - Use only the `NNN-*.md` prefix in `plans/`, `plans/completed/`, and `plans/history/` to compute the next number. Increment by 1 and zero-pad to 3 digits.
