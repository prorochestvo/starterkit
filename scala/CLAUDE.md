...
agent (`scala-testdoctor`) is invoked on-demand whenever tests fail, at any stage.
...
3. scala-reviewer
    ...
    ↓
  ❌ Problems?       → Back to scala-engineer with specific findings
  ⚠️  Tests failing? → scala-testdoctor diagnoses and patches, then rerun reviewer
...
| `scala-reviewer` | Verdicts, severity-ranked findings, patches | Review report; moves plan to `completed/` on approval |
| `scala-testdoctor` | Triage of failing tests, minimal patches | Code/test fixes, re-run of `sbt test` |

### Rules

- **No skipping stages.** Every task starts with the architect and ends with reviewer
  approval.
- **Plan file first.** The architect MUST produce a plan file before any code is
  written. If a plan already exists for the task, update it rather than creating a
  new one.
- **Review loop.** Engineer ↔ Reviewer cycle repeats until the reviewer approves.
- **Reviewer gates completion.** Only the reviewer moves the plan to `plans/completed/`
  with the `YYMMDD.NNNN.slug.md` rename.
- **`sbt compile` and `sbt test` (and `sbt scalafmtCheckAll`) must pass** before the
  reviewer gives approval. If tests fail, hand the logs to `scala-testdoctor`.
- **Testdoctor is scoped.** It patches tests or the minimal production code needed to
  make the failure go away. It does not redesign or refactor.
